	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<?php if ( is_single() ) : ?>
			<h1 class="entry-title"><?php _e( 'Link', 'hdn' ); ?>: <?php the_title(); ?></h1>
		<?php else : ?>
			<h1 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'hdn' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php _e( 'Link', 'hdn' ); ?>: <?php the_title(); ?></a></h1>
		<?php endif; // is_single() ?>
