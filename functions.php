...
/* Insert Sass or CSS
 * all the cleaning and enqueue if you Sass to customize hdn
 * */
...
function hdn_theme_support() {
	// Add language supports. Please note that hdn does not include language files, not yet
	...
	register_nav_menus(array(
		'primary'		=>	__( 'Main Navigation', 'hdn' ),
		'header-menu'  	=>	__( 'Header Menu', 'hdn' ),
		'footer-menu' 	=>	__( 'Footer Menu', 'hdn' )
	)); // Add menu supports
...
add_action('after_setup_theme', 'hdn_theme_support'); 
...
function hdn_entry_meta() {
	...
	if ( $tag_list ) {
		$utility_text = __( '%1$s / %2$s / <span class="by-author"> by %4$s</span>', 'hdn' );
...
if ( ! function_exists( 'hdn_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * To override this walker in a child theme without modifying the comments template
 * simply create your own hdn_comment(), and that function will be used instead.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function hdn_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'hdn' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Edit)', 'hdn' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
			...
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<cite class="fn">%1$s %2$s</cite>',
						...
						( $comment->user_id === $post->post_author ) ? '<span> ' . __( 'Post author', 'hdn' ) . '</span>' : ''
					);
					printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
						esc_url( get_comment_link( $comment->comment_ID ) ),
						get_comment_time( 'c' ),
						/* translators: 1: date, 2: time */
						sprintf( __( '%1$s at %2$s', 'hdn' ), get_comment_date(), get_comment_time() )
					);
				?>
			</header><!-- .comment-meta -->

			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'hdn' ); ?></p>
			<?php endif; ?>

			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( 'Edit', 'hdn' ), '<p class="edit-link">', '</p>' ); ?>
			</section><!-- .comment-content -->

			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'hdn' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
		break;
...
function hdn_credits() {
	printf(
		__('Copyright &copy; %1$s <a href="%2$s">%3$s</a> - All Rights Reserved.', 'hdn'),
		date( 'Y' ),
		home_url( '/' ),
		get_bloginfo( 'name' )
	);
